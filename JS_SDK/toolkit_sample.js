...
//options.toolkitConfig = 'vg_url = http://172.16.11.163/vg-server/ValidationGateway/WireFormatServlet \n'
...
var listReaderCB = function (response, error) {
    //alert("Connecting to reader-listReaderCB");
	if (error !== null) {
...
var connectReaderCB = function (response, error) {
    //alert("Getting Interface - connectReaderCB")
	if (null != error) {
...
var getInterfaceCB = function (response, error) {
    //alert("Web socket connected - getinterfacedb");
	if (null != error) {
		alert(error.code + ' : ' + error.message);
		ToolkitOB = null;
		return;
 ...
 */
function Initialize() {
    //alert("calling initilize function");
    displayProgress('Starting to initialize ...');
    try {
        //alert("try block");
		//showLoader();
...
function displayData(response, div) {
	...
	document.getElementById('PhotoBase64').src = "data:image/bmp;base64," + response.cardHolderPhoto;
